...
t = np.linspace(0, T, 100)
w1 = 2*np.pi*3/T  # f1 = 1
w2 = 10*np.pi*4/T # f2 = 5
wt = 49*2*np.pi*2/T # ft = 100 will cause alias
wt = 100*2*np.pi/T
f = np.sin(w1*t) + np.sin(w2*t) + np.sin(wt*t)
#f  = np.sin(wt*t)
#ff = pylab.fft(f, N)
...
file_name = '/Users/yizhou/teaching/course/1_ComputerVision/images/canoe.tif'
...
#img_cv = ndimage.gaussian_filter(img, sigma=2)
img_cv = ndimage.gaussian_laplace(img, sigma=3)
axes[1].imshow(img_cv, cmap='gray')
axes[1].set_title('convolve gaussian laplace kernel')
...
in_mask = np.zeros((h, w), dtype=np.float32)
in_mask[int(h/2), int(w/2)] = 1
img_kernel = ndimage.gaussian_laplace(in_mask, sigma=3)
